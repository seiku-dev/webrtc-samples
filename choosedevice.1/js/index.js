...
function switchVideoDevice2(){
    var videoIndex = document.getElementById('videoIndex').selectedIndex;
    var audioIndex = document.getElementById('audioIndex').selectedIndex;
    var speakerIndex = document.getElementById('speakerIndex').selectedIndex;
    var setDefault = document.getElementById('setDefault').value;

    RTC.stopRTC({}, function(){
        console.debug('stop succ');
        if(videoDevices.length > 1){
            console.log("执行切换摄像头",videoDevices[videoIndex]);
            RTC.chooseVideoDevice(videoDevices[videoIndex]);
        } else{
            alert("当前设备不支持切换摄像头");
        }
        if(audioDevices.length > 1){
            console.log("执行切换录音设备",audioDevices[audioIndex]);
            // RTC.chooseAudioDevice(audioDevices[audioIndex]);
        } else{
            alert("当前设备不支持切换录音设备");
        }
        // if(speakerDevices.length > 1){
        //     $("#remoteVideo").each(function(index, video){
        //         console.log("打印video信息",video);
        //         RTC.chooseSpeakerDevice( video, speakerDevices[speakerIndex],function(){
        //             console.log("切换speakerDevices成功");
        //         },function(){});
        //     })
        // } else {
        //     alert("当前设备不支持切换扬声器");
