...
WebRTCAPI.fn.chooseVideoDevice = function( video ) {
    ...
    if( Browser.type === 'safari' && Browser.isIPhone){
        global.constraints.video
    }
...
WebRTCAPI.fn.chooseSpeakerDevice = function( mediaElement, speaker , succ,  fail ) {
    console.error('mediaElement.sinkId',mediaElement.sinkId,mediaElement)
    if (typeof mediaElement.sinkId !== 'undefined') {
        mediaElement.setSinkId( speaker.deviceId )
        .then(() => {
            console.error(`Success, audio output device attached: ${sinkId} to element with ${element.title} as source.`);
            succ && succ({
                errorCode: 0,
...
RTCUtil.fn.updateLocalStream = function (
  ...
  console.error('updateLocalStream')
